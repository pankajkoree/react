...
```txt
were familier with all of these as its fundamental for almost every programming languages
but noticied something for the first time in js
as for the remainder: if denominator will be 0, it doesn't throw an error rather it gives the result as NaN
```
